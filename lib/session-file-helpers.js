...
var helpers = {
  ...
  get: function (sessionId, options, callback) {
    ...
    operation.attempt(function () {
      ...
      fs.readFile(sessionPath, options.encrypt && !options.encryptEncoding ? null : options.encoding, function readCallback(err, data) {

        if (!err) {
          var json;
          try {
            json = options.decoder(options.encrypt ? helpers.decrypt(options, data, sessionId) : data);
          } catch (err2) {
            err = err2;
          }
          if (!err) {
  ...
  encrypt: function (options, data, sessionId) {
    ...
    var crypted = options.encryptEncoding
      ? cipher.update(data, options.encoding, options.encryptEncoding )
      : cipher.update(data, options.encoding );
    var cryptedFinal = options.encryptEncoding
      ? cipher.final(options.encryptEncoding)
      : cipher.final();
  ...
  decrypt: function (options, data, sessionId) {
    ...
    var dec = options.encoding
      ? decipher.update(data, options.encryptEncoding, options.encoding)
      : decipher.update(data, options.encryptEncoding);
    var decFinal = options.encoding
