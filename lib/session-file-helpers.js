...
var helpers = {
  ...
  get: function (sessionId, options, callback) {
    ...
    operation.attempt(function () {
      fs.readFile(sessionPath, 'utf8', function (err, data) {
        ...
        if (!err) {
          var json = JSON.parse(options.encrypt ? this.decrypt(data, sessionId) : data);
  ...
  set: function (sessionId, session, options, callback) {
    try {
      ...
      if (options.encrypt) {
        json = this.encrypt(json, sessionId)
      }
  ...
  encrypt: function (text, sessionId) {
    var cipher = crypto.createCipher(this.encAlgorithm,sessionId)
  ...
  decrypt: function (text, sessionId) {
    var decipher = crypto.createDecipher(this.encAlgorithm,sessionId)
