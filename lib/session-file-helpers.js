...
var helpers = {
  ...
  sessionPath: function (basepath, sessionId) {
    ...
    return path.join(basepath, sessionId + fileExtension);
  },

  sessionId: function (file) {
    //return file.substring(0, file.lastIndexOf('.json'));
    return file.replace(options.filePattern, '');
  },
  ...
  getFilePatternFromFileExtension: function (fileExtension) {
    return new RegExp( escapeForRegExp(options.fileExtension) + '$' );
  },
  ...
  defaults: function (options) {
    ...
    return {
      ...
      encode: options.encoder || JSON.stringify,
      decode: options.decoder || JSON.parse,
      ...
      encryptEncoding: options.encryptEncoding || 'hex',
  ...
  get: function (sessionId, options, callback) {
    ...
    operation.attempt(function () {
      fs.readFile(sessionPath, { encoding: options.encoding }, function (err, data) {

        if (!err) {
          var json;
          try {
            json = options.decode(options.encrypt ? helpers.decrypt(data, sessionId) : data);
          } catch (err2) {
            err = err2;
          }
          if (!err) {
  ...
  set: function (sessionId, session, options, callback) {
    try {
      ...
      var sessionPath = helpers.sessionPath(options.path, sessionId);
      var json = options.encode(session);
      if (options.encrypt) {
  ...
  encrypt: function (text, sessionId) {
    ...
    var crypted = cipher.update(text, options.encoding, options.encryptEncoding );
  ...
  decrypt: function (text, sessionId) {
    ...
    var dec = decipher.update(text, options.encryptEncoding, options.encoding);
    var decFinal = decipher.final(options.encryptEncoding);
